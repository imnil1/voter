...
// Extracts the Turnstile sitekey from the page.
//
// Same root cause as isTurnstilePresent's original bug: the iframe that
// carries the sitekey in its src lives behind a closed shadow root
// (confirmed via captured page HTML from a real occurrence), so
// `document.querySelectorAll("iframe")` can never see it — this isn't a
// timing issue, and page.content()/CDP snapshots don't help either, since
// closed shadow roots are explicitly excluded from serialization by spec
// unless the calling code already holds a reference to the root (which we
// don't — that's what "closed" means).
//
// The sitekey still has to exist as a literal string somewhere before
// Cloudflare's JS builds the shadow root client-side — Cloudflare sitekeys
// are a stable, recognizable shape (0x + 22 base62 chars, e.g.
// "0x4AAAAAAA-TQ7IDP_8kRlq5"), so the reliable extraction point is the raw
// page HTML source (page.content()), not the live rendered DOM: any inline
// script or server-rendered attribute referencing the widget will contain
// it in plain text, light-DOM-only or not.
async function getTurnstileSitekey(page) {
  const SITEKEY_PATTERN = /0x[0-9a-zA-Z_-]{20,24}/;

  try {
    // Still try the light-DOM element first — cheap, and correct if
    ...
    // Fall back to the raw page source — this is what actually works
    // against top.gg's closed-shadow-root embed.
    const html = await page.content();
    const match = html.match(SITEKEY_PATTERN);
