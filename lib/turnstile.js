...
//
// DIAGNOSTIC (temporary): a real Turnstile widget was visibly on-screen in
// production (confirmed via debug screenshot) while this returned false for
// the full duration of two separate poll windows — meaning no iframe's src
// matched "challenges.cloudflare.com" at check time. Rather than guess why
// (lazy/deferred src, a nested/proxied frame, top.gg rewriting the src
// through their own domain), this now logs every iframe src it actually
// sees whenever the match fails, so the next real occurrence tells us
// what's actually there instead of just "not found". Remove once the real
// src pattern is confirmed and the match condition is fixed accordingly.
async function isTurnstilePresent(page, shortT = "?") {
  try {
    const { found, allSrcs } = await page.evaluate(() => {
      const iframes = [...document.querySelectorAll("iframe")];
      const srcs = iframes.map(f => f.src || "(no src)");
      const found = iframes.some(f => (f.src || "").includes("challenges.cloudflare.com"));
      return { found, allSrcs: srcs };
    });
    if (!found && allSrcs.length > 0) {
      logVerbose(`[turnstile-diag:${shortT}] no cloudflare-matching iframe; ${allSrcs.length} iframe(s) present: ${JSON.stringify(allSrcs)}`);
    } else if (!found) {
      logVerbose(`[turnstile-diag:${shortT}] no iframes present at all on this check`);
    }
    return found;
  } catch (err) {
