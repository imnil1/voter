...
//
// Root cause (confirmed twice over): the sitekey lives only inside the
// challenge iframe's src — that iframe sits behind a CLOSED shadow root,
// invisible to any DOM query. And a full raw-HTML dump from a real
// production page, searched byte-for-byte, contained no sitekey-shaped
// string anywhere — confirming top.gg's page source never has it
// server-side either; Cloudflare's own script builds the widget (sitekey
// included) entirely client-side, at runtime, only when it decides to
// show a challenge.
//
// The one place the sitekey is genuinely observable is the network request
// Cloudflare's script makes to fetch/render the frame — its URL contains
// the sitekey as a path segment, and request URLs aren't hidden by shadow
// DOM the way DOM contents are. preparePage() (browser.js) attaches a
// request listener that captures this onto the page object as soon as it
// happens; this function just reads it, waiting briefly in case the
// request hasn't fired yet relative to when "captcha" state was detected.
async function getTurnstileSitekey(page, shortT = "?", botId = "?") {
  try {
    // The request may not have fired at the exact instant the caller
    // detected "captcha" state — give it a short window before giving up.
    for (let i = 0; i < 10; i++) {
    ...
    // DIAGNOSTIC (temporary): both the network listener and the DOM
    // fallback came up empty. Dump the raw HTML so the next real
    // occurrence tells us what's actually different, rather than guessing
    // a third time.
    try {
