...
// Extracts the Turnstile sitekey for the current challenge.
//
// Root cause (confirmed multiple times over): the sitekey lives only
// inside the challenge iframe's src — that iframe sits behind a CLOSED
// shadow root, invisible to any DOM query. A full raw-HTML dump from a
// real production page, searched byte-for-byte, contained no sitekey-
// shaped string anywhere — top.gg's page source never has it server-side;
// Cloudflare's own script builds the widget (sitekey included) entirely
// client-side, at runtime, only when it decides to show a challenge.
//
// Two independent capture paths exist for this reason (both set up in
// preparePage, browser.js):
// 1. A network-request listener — catches the sitekey when it appears as
//    a literal substring in a real HTTPS request URL.
// 2. A window.turnstile.render() interception — necessary because a real
//    captured Network tab showed every challenges.cloudflare.com request
//    as a `blob:` URL once actually rendering (Cloudflare builds it
//    in-memory, not via a fetchable URL), and the one genuine HTTPS
//    request present (`api.js?...render=explicit`) confirms top.gg calls
//    Cloudflare's render() explicitly with the sitekey as a plain JS
//    argument — never serialized into any URL at all in that mode.
//
// Both write to page._lastTurnstileSitekey (method 1, set directly on the
// Node-side page object) or window._lastTurnstileSitekey (method 2, set
// in-page — read via evaluate()). Either can fire first depending on
// timing, so both are polled together.
async function getTurnstileSitekey(page, shortT = "?", botId = "?") {
  try {
    for (let i = 0; i < 10; i++) {
      if (page._lastTurnstileSitekey) return page._lastTurnstileSitekey;
      const fromWindow = await page.evaluate(() => window._lastTurnstileSitekey || null).catch(() => null);
      if (fromWindow) return fromWindow;
      await delay(300);
    }
    if (page._lastTurnstileSitekey) return page._lastTurnstileSitekey;

    // Still try the light-DOM element as a cheap fallback — correct if
    // Cloudflare ever does render a [data-sitekey] outside the shadow root.
    ...
    // DIAGNOSTIC (temporary): all three capture paths came up empty. Dump
    // the raw HTML so the next real occurrence tells us what's actually
    // different, rather than guessing further.
    try {
      const { saveDebugHTML } = require("./screenshot");
      await saveDebugHTML(page, shortT, botId, "sitekey-extraction-failed");
    } catch (_) {}
...
async function solveTurnstile(sitekey, pageUrl, shortT) {
  ...
  try {
    // The solver's /in.php reads params via request.post() (form body), not
    // query string — a GET with everything in the URL returns empty params
    // and always gets ERROR_WRONG_PARAMETER. POST with URLSearchParams fixes
    // this without changing the solver side.
    ...
    const submitRes = await fetch(`${SOLVER_URL}/in.php`, {
      method: "POST",
      body: submitBody,
      signal: AbortSignal.timeout(15000),
    });
    const submitText = await submitRes.text();
    // 2captcha-style APIs (in.php/res.php) are known to return plain
    // error text (e.g. "ERROR_WRONG_USER_KEY") even with json=1 set,
    // rather than always wrapping it as {"status":0,"request":"..."}.
    // Parsing text->JSON ourselves means a non-JSON response surfaces as
    // a real error message here, instead of a cryptic "Unexpected token"
    // from .json() with no indication of what the API actually said.
    let submitJson;
    try {
      submitJson = JSON.parse(submitText);
    } catch (_) {
      log(`[turnstile-solver:${shortT}] Submit returned non-JSON response: ${submitText.slice(0, 200)}`);
      return null;
    }
    ...
    while (Date.now() - start < POLL_TIMEOUT_MS) {
      ...
      try {
        ...
        log(`[turnstile-solver:${shortT}] Poll returned non-JSON response: ${pollText.slice(0, 200)}`);
        return null;
      }
...
async function trySolveTurnstileOnPage(page, shortT, botId) {
  const sitekey = await getTurnstileSitekey(page, shortT, botId);
