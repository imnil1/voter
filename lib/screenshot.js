...
// Saves the full rendered HTML (page.content(), i.e. current DOM state,
// not view-source) alongside the debug screenshot for the same failure.
// This exists specifically to answer "what did the Turnstile markup
// actually look like at that moment" without guessing from a screenshot —
// e.g. whether the widget's iframe had a src yet, whether it was nested,
// what class/data attributes its container had. Same directory, same
// safety posture as saveDebugScreenshot: never throws, skips cleanly if
// the page is already closed.
async function saveDebugHTML(page, label, botId, reason) {
  try {
    if (page.isClosed()) {
      log(`[html:${label}] Skipped for bot ${botId} (${reason}) — page already closed`);
      return;
    }
    ...
    const safeLabel = String(label).replace(/[^a-zA-Z0-9_-]/g, "_");
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const filename = `${safeLabel}_${botId}_${reason}_${timestamp}.html`;
    const filepath = path.join(SCREENSHOT_DIR, filename);

    fs.writeFileSync(filepath, html, "utf-8");
    log(`[html:${label}] Saved debug HTML for bot ${botId}: ${filename}`);
  } catch (err) {
    if (/closed/i.test(err.message)) {
      log(`[html:${label}] Could not capture for bot ${botId} (${reason}) — page/context/browser closed before or during capture`);
    } else {
      log(`[html:${label}] Failed to save HTML: ${err.message}`);
    }
  }
}

module.exports = { saveDebugScreenshot, saveDebugHTML };
